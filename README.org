...
| github-token       | PAT token with more information at [[sec:pat][Personal Access Token - PAT]].              |               |
| git-tags           | The repository tags from the newest to the oldest. Example [[sec:extract-tags][Extract the tags]]. |               |
